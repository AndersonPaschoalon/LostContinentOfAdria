 ...
 * @section DESCRIPTION
 *
 * Each players starts with one of these units: 
 * Osiris, Prometheus, Guardian, Gaia, a Titan, and a Living Statue of Posseidon 
 * @TODO
 * - remove <include "MmM_FE_lib.xs";>
 */
...
void main(void){
	...
	for(i=0; <numTries*mapSizeMultiplier){
		...
		{
			terrainType = "Hadesbuildable1";
		}
		else if (terrainCount == 3) 
		{
			terrainType = "grassA";
		}
		else if (terrainCount == 4) 
		{
			terrainType = "grassA";
		}
		else 
		...
		rmSetAreaTerrainType(elevID, terrainType);
	...
	for(i=0; <numPond){
		...
		//rmSetAreaWaterType(smallPondID, "Greek River");
	...
	int osiris=rmCreateObjectDef("Osiris");
	rmAddObjectDefItem(osiris, "Osiris", 1, 3.0);
	rmSetObjectDefMinDistance(osiris, 8.0);
	rmSetObjectDefMaxDistance(osiris, 16.0);
	rmPlaceObjectDefPerPlayer(osiris, true);

	int titanProm=rmCreateObjectDef("Prometheus");
	rmAddObjectDefItem(titanProm, "Titan Prometheus", 1, 3.0);
	rmSetObjectDefMinDistance(titanProm, 8.0);
	rmSetObjectDefMaxDistance(titanProm, 18.0);
	rmPlaceObjectDefPerPlayer(titanProm, true);	

	int guardian=rmCreateObjectDef("Guardian");
	rmAddObjectDefItem(guardian, "Guardian XP", 1, 3.0);
	rmSetObjectDefMinDistance(guardian, 8.0);
	rmSetObjectDefMaxDistance(guardian, 16.0);
	rmPlaceObjectDefPerPlayer(guardian, true);

	int gaia=rmCreateObjectDef("Gaia");
	rmAddObjectDefItem(gaia, "Titan Gaia", 1, 3.0);
	rmSetObjectDefMinDistance(gaia, 10.0);
	rmSetObjectDefMaxDistance(gaia, 25.0);
	rmPlaceObjectDefPerPlayer(gaia, true);

	int titanStatue=rmCreateObjectDef("Titan Statue");
	rmAddObjectDefItem(titanStatue, "Titan Bad", 1, 3.0);
	rmSetObjectDefMinDistance(titanStatue, 10.0);
	rmSetObjectDefMaxDistance(titanStatue, 25.0);
	rmPlaceObjectDefPerPlayer(titanStatue, true);	

	int titan=rmCreateObjectDef("Titan");
	rmAddObjectDefItem(titan, "Titan Atlantean", 1, 3.0);
	rmSetObjectDefMinDistance(titan, 25.0);
	rmSetObjectDefMaxDistance(titan, 26.0);
	rmPlaceObjectDefPerPlayer(titan, true);	
	...
	int avoidTower=rmCreateTypeDistanceConstraint("avoid tower", "tower", 1.0);
	...
	// volcano
	int volcanoID=rmCreateObjectDef("volcano");
	rmAddObjectDefItem(volcanoID, "volcano", 1, 0.0);
	rmSetObjectDefMinDistance(volcanoID, 15.0);
	rmSetObjectDefMaxDistance(volcanoID, 24.0);
	rmAddObjectDefConstraint(volcanoID, avoidTower);
	rmAddObjectDefConstraint(volcanoID, forestTower);
	rmAddObjectDefConstraint(volcanoID, shortAvoidGold);
	rmAddObjectDefConstraint(volcanoID, stragglerTreeAvoid);
	// troy gate
	int troyGateID=rmCreateObjectDef("Starting troy gate");
	rmAddObjectDefItem(troyGateID, "Wall of Troy Short", 1, 2.0);
	rmSetObjectDefMinDistance(troyGateID, 20.0);
	rmSetObjectDefMaxDistance(troyGateID, 20.0);
	rmAddObjectDefConstraint(troyGateID, avoidTower);
	rmAddObjectDefConstraint(troyGateID, forestTower);
	rmAddObjectDefConstraint(troyGateID, shortAvoidGold);
	rmAddObjectDefConstraint(troyGateID, stragglerTreeAvoid);
	rmPlaceObjectDefPerPlayer(troyGateID, true);	
	int firstLoop = 1;
	int placement = 1;
	float increment = 1.0;
	for(p = 1; <= cNumberNonGaiaPlayers){
		...
		increment = 24;
		firstLoop = 1;
		while( rmGetNumberUnitsPlaced(startingTowerID) < (10*p) ){
			rmPlaceObjectDefAtLoc(startingTowerID, p, rmGetPlayerX(p), rmGetPlayerZ(p), 1);	
			rmPlaceObjectDefAtLoc(volcanoID, p, rmGetPlayerX(p), rmGetPlayerZ(p), 1);	
			placement++;
			if(placement % 20 == 0){
				increment++;
