...
// Main entry point for random map script
  ...
  // Text
   ...
   // Init map.
   rmSetSeaType("Norwegia Sea");
   
   //rmTerrainInitialize("SnowSand25");
   rmTerrainInitialize("GaiaCreepASnow");
   ...
   rmAddObjectDefItem(closeBerriesID, "berry bush", 4, 2.0);//berry bush
 ...
 //******Cetral Lake*************
   ...
   rmSetAreaSize(coreOneID, 0.08, 0.08);
   //rmSetAreaSize(coreOneID, 0.06, 0.06);
   rmSetAreaLocation(coreOneID, 0.8, 0.2);
   rmSetAreaWaterType(coreOneID, "Norwegian Sea");
   ...
   rmAddObjectDefItem(farGoldID, "Gold mine", 5, 6.0);
   ...
   // Gaia Forest 
   int gaiaForestID=rmCreateObjectDef("gaia forest");
   rmAddObjectDefItem(gaiaForestID, "Gaia Forest tree", 800, 20000.0);
   rmSetObjectDefMinDistance(gaiaForestID, 0.0);
   rmSetObjectDefMaxDistance(gaiaForestID, 0.01);
   rmAddObjectDefConstraint(gaiaForestID, avoidImpassableLand);
   rmAddObjectDefConstraint(gaiaForestID, farStartingSettleConstraint);
  ...
  for(i=1; <numIsland)
  {
      ...
      rmSetAreaTerrainType(bonusID, "GaiaCreepASnow");
      //rmAddAreaTerrainLayer(bonusID, "snowB", 0, 3);            
      rmAddAreaTerrainLayer(bonusID, "GaiaCreepASnow", 0, 3);
   }
...
*/ 
   ...
   for(i=1; <cNumberPlayers)
   {
      ...
      rmSetAreaTerrainType(id, "GaiaCreepASnow");
      rmAddAreaTerrainLayer(id, "GaiaCreepASnow", 4, 6);
      rmAddAreaTerrainLayer(id, "GaiaCreepASnow", 2, 4);
      rmAddAreaTerrainLayer(id, "GaiaCreepASnow", 0, 2);     
   }
 ...
 /*  // Player sub-area
   ...
   for(i=1; <cNumberPlayers*2) 
   {
      ...
      rmSetAreaTerrainType(patch, "GaiaCreepASnow");
   ...
   for(i=0; <numTries)
   {
      ...
      if(rmRandFloat(0.0, 1.0)<0.5){
         rmSetAreaTerrainType(elevID, "GaiaCreepASnow");
...
/*
   ...
   if(rmPlaceFairLocs())
   {
      ...
      //rmAddObjectDefItem(fortressID, "Healing Spring", 1, 15.0);
      ...
      //rmAddObjectDefItem(migdolID, "Healing Spring", 1, 15.0);
      ...
      //rmAddObjectDefItem(hillFortID, "Healing Spring", 1, 15.0);
      ...
      //rmAddObjectDefItem(palaceID, "Healing Spring", 1, 15.0);
...
*/
   ...
   if(rmPlaceFairLocs())
   {
      templeID=rmCreateObjectDef("player temple");
      rmAddObjectDefItem(templeID, "Temple", 1, 0.0);
      rmAddObjectDefItem(templeID, "Gold Mine Small", 1, 8.0);

      for(i=1; <cNumberPlayers)
      {
         for(j=0; <rmGetNumberFairLocs(i))
         {
            rmPlaceObjectDefAtLoc(templeID, i, rmFairLocXFraction(i, j), rmFairLocZFraction(i, j), 1);
...
/*
   ...
   int HealingSpringID=0;

   if(rmPlaceFairLocs())
   {
      HealingSpringID=rmCreateObjectDef("player healing spring");
      rmAddObjectDefItem(HealingSpringID, "Healing Spring", 1, 0.0);
      rmAddObjectDefItem(HealingSpringID, "Gold Mine Small", 1, 8.0);

      for(i=1; <cNumberPlayers)
      {
         for(j=0; <rmGetNumberFairLocs(i))
         {
            rmPlaceObjectDefAtLoc(HealingSpringID, i, rmFairLocXFraction(i, j), rmFairLocZFraction(i, j), 1);
...
/*   rmPlaceObjectDefPerPlayer(startingGoldID, false); */
   ...
   //rmPlaceObjectDefPerPlayer(closeGaiaTreeID, false);
   ...
   // Berries.
   rmPlaceObjectDefAtLoc(gaiaForestID, 0, 0.5, 0.5, cNumberPlayers/2);
